...
- **2026-08-06** — Evolução do #5 (DnD no Kanban) para mobile: substituído `PointerSensor` (disparava drag por toque em qualquer movimento de rolagem) por `MouseSensor` (desktop, `distance: 8`) + `TouchSensor` (mobile, **long-press de 1000ms** com `tolerance: 5px` para não roubar a rolagem). Cards sortable trocaram `touch-none` por `touch-pan-y select-none` — agora rolar a página na área do Kanban não move mais leads; mover exige segurar ~1s. KeyboardSensor inalterado. Typecheck, ESLint e `npm run test:crm` (79/79) passando.
